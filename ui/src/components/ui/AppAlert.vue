<!--
  AppAlert.vue — Inline contextual message block
  ───────────────────────────────────────────────────────────────
  Props:
    type     : 'success' | 'error' | 'warning' | 'info'  default 'info'
    message  : string (also available as default slot)

  Usage:
    <AppAlert type="error" :message="errorMsg" v-if="errorMsg" />
    <AppAlert type="success">Data berhasil disimpan.</AppAlert>
-->
<template>
  <div
    v-if="hasContent"
    :class="['flex items-start gap-3 rounded-lg px-4 py-3 text-sm', STYLES[type].bg]"
    role="alert"
  >
    <!-- Icon -->
    <svg
      :class="['w-5 h-5 flex-shrink-0 mt-0.5', STYLES[type].icon]"
...
const hasContent = computed(() => props.message || slots.default)
