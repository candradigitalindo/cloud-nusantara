<!--
  ProcurementDashboard.vue — Global Procurement Dashboard
  Route: /procurement-dashboard   meta: { permission: 'procurement.view' }
-->
<template>
  <div class="pd-root">
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-sub">Ringkasan seluruh pengadaan unit bisnis</p>
      </div>
    ...
    <!-- ── Loading skeleton ── -->
    <div v-if="loading && !data" class="skel-grid">
      <div v-for="i in 4" :key="i" class="skel-card"><div class="skel-bar w60" /><div class="skel-bar w40" /></div>
    </div>
    ...
    <template v-if="data">
      ...
      <div class="stat-grid">
        <div v-for="card in statCards" :key="card.label" :class="['stat-card', `sc-${card.theme}`]">
          ...
          <div class="sc-footer">
            <span v-if="card.sub" class="sc-tag">{{ card.sub }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="chart-row">
        ...
        <div class="chart-panel">
          ...
          <div class="chart-wrap chart-doughnut-wrap">
            ...
            <span v-else class="chart-empty">Belum ada data</span>
          </div>
        ...
        <div class="chart-panel">
          ...
          <div class="chart-wrap chart-doughnut-wrap">
            <Doughnut v-if="hasTypeData" :data="typeChartData" :options="doughnutOptions" />
            <span v-else class="chart-empty">Belum ada data</span>
          </div>
          <div class="chart-legend">
      ...
      <div class="chart-panel chart-panel--wide">
        <div class="cp-header">
          ...
          <h3 class="cp-title">Tren Pengadaan (12 Bulan Terakhir)</h3>
        </div>
      ...
      <div class="wu-panel">
        ...
        <div v-if="data.work_units.length === 0" class="wu-empty">
          <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" opacity=".35"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
          <span>Belum ada data pengadaan per unit kerja.</span>
        </div>
        ...
        <template v-else>
            ...
            <table class="wu-table">
              <thead>
                <tr>
                  ...
                  <th class="text-center">Minta Bayar</th>
              ...
              <tbody>
                <tr v-for="wu in data.work_units" :key="wu.work_unit_id">
                  <td>
                    <div class="wu-name-cell">
                      <div class="wu-avatar">{{ wu.work_unit_name.charAt(0).toUpperCase() }}</div>
                      <span>{{ wu.work_unit_name }}</span>
                    </div>
                  </td>
                  <td class="text-right fw600">{{ formatRupiah(wu.total_amount) }}</td>
                  <td class="text-center fw600">{{ wu.total }}</td>
                  <td class="text-center"><span v-if="wu.pending" class="badge badge-pending">{{ wu.pending }}</span><span v-else class="muted">0</span></td>
                  <td class="text-center"><span v-if="wu.approved" class="badge badge-approved">{{ wu.approved }}</span><span v-else class="muted">0</span></td>
                  <td class="text-center"><span v-if="wu.payment_requested" class="badge badge-payment-requested">{{ wu.payment_requested }}</span><span v-else class="muted">0</span></td>
                  <td class="text-center"><span v-if="wu.paid" class="badge badge-paid">{{ wu.paid }}</span><span v-else class="muted">0</span></td>
                  <td class="text-center"><span v-if="wu.received" class="badge badge-received">{{ wu.received }}</span><span v-else class="muted">0</span></td>
                  <td class="text-center"><span v-if="wu.rejected" class="badge badge-rejected">{{ wu.rejected }}</span><span v-else class="muted">0</span></td>
                  <td class="text-center"><span v-if="wu.cancelled" class="badge badge-cancelled">{{ wu.cancelled }}</span><span v-else class="muted">0</span></td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <!-- Mobile cards -->
          <div class="wu-mobile">
            <div v-for="wu in data.work_units" :key="wu.work_unit_id" class="wmc">
              <div class="wmc-top">
                <div class="wu-avatar">{{ wu.work_unit_name.charAt(0).toUpperCase() }}</div>
                <div class="wmc-info">
                ...
                <span class="wmc-total">{{ wu.total }}</span>
              </div>
              <div class="wmc-badges">
                ...
                <span v-if="wu.payment_requested" class="badge badge-payment-requested">Minta Bayar {{ wu.payment_requested }}</span>
                ...
                <span v-if="wu.cancelled" class="badge badge-cancelled">Batal {{ wu.cancelled }}</span>
              </div>
              <div class="wmc-bar">
                <div class="bar-seg seg-received" :style="{ width: pct(wu.received, wu.total) }" />
                <div class="bar-seg seg-paid"     :style="{ width: pct(wu.paid, wu.total) }" />
                <div class="bar-seg seg-payment-requested" :style="{ width: pct(wu.payment_requested, wu.total) }" />
                <div class="bar-seg seg-approved"  :style="{ width: pct(wu.approved, wu.total) }" />
                <div class="bar-seg seg-pending"   :style="{ width: pct(wu.pending, wu.total) }" />
                <div class="bar-seg seg-rejected"  :style="{ width: pct(wu.rejected + wu.cancelled, wu.total) }" />
              </div>
            </div>
          </div>
        </template>
...
async function fetchData() {
  ...
  errorMsg.value = ''
  try {
    data.value = await purchaseApi.getDashboard()
  } catch (err) {
...
const statCards = computed(() => {
  ...
  const d = data.value
  const done = d.status_breakdown.received + d.status_breakdown.paid
  return [
    {
      label: 'Total Pengadaan', theme: 'blue',
      value: d.total_requests.toLocaleString('id-ID'),
      sub: `${done} selesai / dibayar`,
    ...
    {
      label: 'Nominal Total', theme: 'emerald',
    ...
    {
      label: 'Ditolak / Dibatalkan', theme: 'red',
    ...
    {
      label: 'Tagihan', theme: 'orange',
      value: formatRupiah(d.accounts_payable?.total_amount || 0),
      sub: `${d.accounts_payable?.count || 0} pengadaan belum dibayar`,
      svg: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>`,
    },
  ]
})
...
// ── Status chart ──
const STATUS_COLORS = {
  pending: '#f59e0b', approved: '#3b82f6', payment_requested: '#f97316', partial: '#d97706',
  paid: '#8b5cf6', received: '#10b981', rejected: '#ef4444', cancelled: '#6b7280',
}
const STATUS_LABELS = {
  pending: 'Pending', approved: 'Disetujui', payment_requested: 'Menunggu Bayar', partial: 'Dibayar Sebagian',
  paid: 'Dibayar', received: 'Diterima', rejected: 'Ditolak', cancelled: 'Dibatalkan',
}

const hasStatusData = computed(() => data.value && data.value.total_requests > 0)
const hasTypeData   = computed(() => data.value && (data.value.type_split.barang > 0 || data.value.type_split.jasa > 0))
...
.pd-refresh {
  ...
  padding: .38rem .9rem; border-radius: .6rem; cursor: pointer;
...
/* ── Skeleton ── */
.skel-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: .875rem; }
.skel-card {
...
.sc-orange  .sc-blob { background: #f97316; }
...
.sc-orange  .sc-icon-wrap { background: rgba(249,115,22,.12);  color: #ea580c; }
...
.chart-empty { font-size: .8rem; color: #8a9e93; font-weight: 500; }
...
.wu-table th {
  ...
  background: rgba(248,252,249,.6); white-space: nowrap;
}
.wu-table td {
...
.badge-payment-requested { background: rgba(249,115,22,.13); color: #c2410c; }
...
.wu-mobile { display: none; }
.wmc {
  padding: .9rem 1.1rem; border-bottom: 1px solid rgba(0,0,0,.05);
}
.wmc:last-child { border-bottom: none; }
.wmc-top { display: flex; align-items: center; gap: .5rem; margin-bottom: .5rem; }
...
.wmc-total {
  font-size: .72rem; color: #fff; font-weight: 700; white-space: nowrap;
...
/* Mini progress bar */
.wmc-bar {
  display: flex; height: 6px; border-radius: 3px; overflow: hidden; background: rgba(0,0,0,.06);
}
.bar-seg { height: 100%; transition: width .3s ease; }
.seg-received { background: #10b981; }
.seg-paid     { background: #8b5cf6; }
.seg-payment-requested { background: #f97316; }
.seg-approved { background: #3b82f6; }
