<template>
  <div class="space-y-5">
    <div class="flex items-center justify-between">
      <h1 class="text-xl font-bold text-gray-900">Pengadaan Jasa</h1>
    ...
    <AppCard :padding="false">
      <AppTable :columns="COLUMNS" :rows="requests" :loading="loading" emptyText="Belum ada pengajuan jasa.">
    ...
    <!-- Create Modal -->
    <AppModal v-model="showCreate" title="Pengajuan Jasa / Layanan" size="2xl">
      <form class="space-y-4" @submit.prevent="submitCreate">
            ...
            <div v-for="(item, i) in form.items" :key="i" class="bg-gray-50 rounded-lg p-4 border border-gray-200">
              <div class="flex items-start gap-2">
                <div class="flex-1 space-y-3">
                  ...
                  <input v-model="item.name" placeholder="Nama Pengadaan Jasa" class="input-sm w-full" />
                  <!-- Sub-items -->
        ...
        <AppInput v-model="form.notes" label="Catatan / Scope of Work" placeholder="Deskripsi pekerjaan, jadwal, dsb. (opsional)" />
      </form>
      <template #footer>
    ...
    <!-- Detail Modal -->
    <AppModal v-model="showDetail" title="Detail Pengajuan Jasa" size="2xl">
              ...
              <div :class="detail.children?.length ? '' : 'grid grid-cols-1 sm:grid-cols-2 gap-4'">
                ...
                <button v-if="!detail.children?.length" @click="openEditFinal" class="flex flex-col items-center gap-3 p-5 bg-white border-2 border-emerald-100 hover:border-emerald-500 rounded-2xl transition-all group text-left shadow-sm">
                  ...
                  <div class="text-center">
                    ...
                    <p class="text-[11px] text-gray-500 leading-tight mt-1">Gunakan satu supplier untuk seluruh daftar jasa di atas.</p>
                  </div>
                </button>
                ...
                <button @click="openSplitVendor" class="flex flex-col items-center gap-3 p-5 bg-white border-2 border-blue-50 hover:border-blue-500 rounded-2xl transition-all group text-left shadow-sm">
                  ...
                  <div class="text-center">
                    ...
                    <p class="text-[11px] text-gray-500 leading-tight mt-1">Pisahkan beberapa jasa ke supplier lain (Split Invoice).</p>
                  </div>
                </button>
              </div>
            </div>
      ...
      <template #footer>
        <div v-if="!parentDetailId" class="flex items-center gap-2 mr-auto">
          ...
          <AppButton v-if="detail && detail.status === 'paid' && authStore.hasPermission('procurement.requests.submit')" variant="primary" @click="confirmDetailAction('receive')">Selesaikan Jasa</AppButton>
    ...
    <AppModal v-model="showEditFinal" title="Isi Harga (Purchasing)" size="2xl">
      ...
      <p class="text-sm text-gray-500 mb-3">Masukkan harga per satuan untuk setiap item jasa.</p>
      <div class="space-y-4">
    ...
    <!-- Split Vendor Modal -->
    <AppModal v-model="showSplitModal" title="Split Jasa ke Vendor Lain" size="2xl">
      <div class="space-y-4">
        <AppAlert type="error" :message="splitError" />
        <p class="text-sm text-gray-500">Pilih item jasa yang ingin Anda pindahkan ke vendor/supplier berbeda.</p>
    ...
    <AppModal v-model="showActionConfirm" :title="actionConfirmTitle" size="sm">
      <div class="flex items-start gap-3">
        <div :class="actionConfirmIconBg" class="shrink-0 w-10 h-10 rounded-full flex items-center justify-center">
          ...
          <svg v-else class="w-5 h-5 text-purple-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
        </div>
        <div>
...
const ACTION_META = {
  approve:         { title: 'Setujui Pengajuan',     msg: 'Yakin ingin menyetujui pengajuan jasa ini?',               btn: 'Setujui',           bg: 'bg-blue-100',    done: 'disetujui' },
  request_payment: { title: 'Ajukan Pembayaran',     msg: 'Ajukan pembayaran ke tim keuangan untuk pengajuan jasa ini?', btn: 'Ajukan Pembayaran', bg: 'bg-orange-100',  done: 'diajukan untuk pembayaran' },
  pay:             { title: 'Bayar Pengajuan',        msg: 'Konfirmasi pembayaran untuk pengajuan jasa ini?',           btn: 'Bayar',             bg: 'bg-emerald-100', done: 'dibayar' },
  receive:         { title: 'Selesaikan Jasa',        msg: 'Konfirmasi jasa sudah diselesaikan?',                      btn: 'Selesai',           bg: 'bg-purple-100',  done: 'diselesaikan' },
}
...
async function submitSplit() {
  ...
  try {
    ...
    toast.success('Beberapa jasa berhasil di-split ke vendor baru.')
...
const statusMap = {
  ...
  received:          { label: 'Selesai',             cls: 'bg-purple-100 text-purple-700' },
...
function addItem() {
  form.value.items.push({ name: '', items: [{ name: '', qty: 1, unit: 'paket', hps_price: 0 }] })
}
function removeItem(i) { if (form.value.items.length > 1) form.value.items.splice(i, 1) }
function addSubItem(i) { form.value.items[i].items.push({ name: '', qty: 1, unit: 'paket', hps_price: 0 }) }
...
async function submitCreate() {
  ...
  if (!validItems.length) { createError.value = 'Minimal 1 jasa harus diisi.'; return }
  ...
  try {
    await purchaseApi.create({
      ...
      vendor_name: form.value.vendor_id ? (vendorList.value.find(v => v.id === form.value.vendor_id)?.name || '') : '',
      items: validItems,
      notes: form.value.notes.trim(),
    })
    toast.success('Pengajuan jasa berhasil dikirim.')
